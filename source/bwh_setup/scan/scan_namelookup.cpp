#include "scan_namelookup.h"
#include "scan_common.h"
#include <commctrl.h>

#define SCAN_CATEGORY "BWH"
#define SCAN_NAME     "NameLookup"
#define SCAN_REQUIRED FALSE

VOID scanNameLookup_ScanProc(INT iItem, HWND hwndDlg, PBYTE pbFile)
{
	...
	// scan data section for "Unknown Being"
	for(DWORD dwDataIndex = 0; dwDataIndex < dwDataSize - (sizeof("Unknown Being") - 1); dwDataIndex++)
	{
		if(MemoryCompare(&pbData[dwDataIndex], (PBYTE)"Unknown Being", sizeof("Unknown Being") - 1))
		{
			...
			if(dwStringVA)
			{
				...
				// scan code section for 'mov eax, dwStringVA'
				for(DWORD dwCodeIndex = 0; dwCodeIndex < dwCodeSize - 8; dwCodeIndex++)
				{
					if(pbCode[dwCodeIndex] == 0xB8 && *((PDWORD)&pbCode[dwCodeIndex + 1]) == dwStringVA)
					{
...
VOID scanNameLookup_Init(HWND hwndList)
{
	...
	ppd->dwValue = 0;
	ppd->pfnScanProc = scanNameLookup_ScanProc;
