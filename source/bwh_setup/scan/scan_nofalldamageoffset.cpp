#include "scan_nofalldamageoffset.h"
#include "scan_common.h"
#include <commctrl.h>

#define SCAN_CATEGORY "BWH"
#define SCAN_NAME     "NoFallDamageOffset"
#define SCAN_REQUIRED FALSE

VOID scanNoFallDamageOffset_ScanProc(INT iItem, HWND hwndDlg, PBYTE pbFile)
{
										...
										for(DWORD dwCodeIndex5 = dwCodeIndex4; dwCodeIndex5 >= dwJumpOffset; dwCodeIndex5--)
										{
											if(pbCode[dwCodeIndex5] == 0x89)
											{
												...
												for(DWORD dwCodeIndex6 = dwCodeIndex5; dwCodeIndex6 >= dwJumpOffset; dwCodeIndex6--)
												{
													...
													{
														if(pbCode[dwCodeIndex6 + 1] & 0x80)
															GetPPD(iItem, hwndDlg)->dwValue = *((PDWORD)&pbCode[dwCodeIndex6+2]);
														else
															GetPPD(iItem, hwndDlg)->dwValue = (DWORD)pbCode[dwCodeIndex6+2];
...
VOID scanNoFallDamageOffset_Init(HWND hwndList)
{
	...
	ppd->dwValue = 0;
	ppd->pfnScanProc = scanNoFallDamageOffset_ScanProc;
