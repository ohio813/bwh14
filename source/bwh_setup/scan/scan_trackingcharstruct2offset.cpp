#include "scan_trackingcharstruct2offset.h"
#include "scan_common.h"
#include <commctrl.h>

#define SCAN_CATEGORY "BWH"
#define SCAN_NAME     "TrackingCharStruct2Offset"
#define SCAN_REQUIRED FALSE

VOID scanTrackingCharStruct2Offset_ScanProc(INT iItem, HWND hwndDlg, PBYTE pbFile)
{
																			...
																			if(pCallHook[dwCallIndex2] == 0x74 && pCallHook[ pCallHook[dwCallIndex2 + 1] + dwCallIndex2 + 1] == 0xC3)
																			{
																				...
																				if(dwCallIndex3 >= dwCallIndex + 6)
																				{
																					...
																					if(dwCallIndex4 < 100)
																					{
																						...
																						GetPPD(iItem, hwndDlg)->dwValue = *((PDWORD)&pCallHook[dwCallIndex+2]);
...
VOID scanTrackingCharStruct2Offset_Init(HWND hwndList)
{
	...
	ppd->dwValue = 0;
	ppd->pfnScanProc = scanTrackingCharStruct2Offset_ScanProc;
