...
#define SCAN_NAME     "Present"
#define SCAN_REQUIRED TRUE

VOID scanPresent_ScanProc(INT iItem, HWND hwndDlg, PBYTE pbFile)
{
	...
	if(pD3D)
	{
		...
		if(SUCCEEDED(pD3D->CreateDevice(D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, hwndDlg, D3DCREATE_SOFTWARE_VERTEXPROCESSING, &pp, &pDevice)))
		{
			...
			__asm mov eax, [eax]
			__asm mov eax, [eax+44h]
			__asm mov dwPresentRVA, eax
			pDevice->Release();
		}
		pD3D->Release();
	}
	if(dwPresentRVA)
	{
		dwPresentRVA -= PtrToUlong(GetModuleHandle("d3d9.dll"));
		GetPPD(iItem, hwndDlg)->dwValue = dwPresentRVA;
...
VOID scanPresent_Init(HWND hwndList)
{
	...
	ppd->dwValue = 0;
	ppd->pfnScanProc = scanPresent_ScanProc;
