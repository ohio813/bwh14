#include "scan_trackingtrackdataoffset.h"
#include "scan_common.h"
#include <commctrl.h>

#define SCAN_CATEGORY "BWH"
#define SCAN_NAME     "TrackingTrackDataOffset"
#define SCAN_REQUIRED FALSE

VOID scanTrackingTrackDataOffset_ScanProc(INT iItem, HWND hwndDlg, PBYTE pbFile)
{
																			...
																			if(pCallHook[dwCallIndex2] == 0x74 && pCallHook[ pCallHook[dwCallIndex2 + 1] + dwCallIndex2 + 1] == 0xC3)
																			{
																				...
																				if(dwCallIndex3 >= dwCallIndex + 6)
																				{
																					...
																					if(dwCallIndex4 < 100)
																					{
																						...
																						GetPPD(iItem, hwndDlg)->dwValue = *((PDWORD)&pbCode[dwCodeIndex7+2]);
...
VOID scanTrackingTrackDataOffset_Init(HWND hwndList)
{
	...
	ppd->dwValue = 0;
	ppd->pfnScanProc = scanTrackingTrackDataOffset_ScanProc;
