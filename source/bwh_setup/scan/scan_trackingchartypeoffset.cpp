#include "scan_trackingchartypeoffset.h"
#include "scan_common.h"
#include <commctrl.h>

#define SCAN_CATEGORY "BWH"
#define SCAN_NAME     "TrackingCharTypeOffset"
#define SCAN_REQUIRED FALSE

VOID scanTrackingCharTypeOffset_ScanProc(INT iItem, HWND hwndDlg, PBYTE pbFile)
{
																			...
																			if(pCallHook[dwCallIndex2] == 0x74 && pCallHook[ pCallHook[dwCallIndex2 + 1] + dwCallIndex2 + 1] == 0xC3)
																			{
																				...
																				if(dwCallIndex3 >= dwCallIndex + 6)
																				{
																					...
																					if(dwCallIndex4 < 100)
																					{
																						...
																						if((pCallHook[dwCallIndex4+2] & 0x80) == 0x80)
																						{
																							GetPPD(iItem, hwndDlg)->dwValue = *((PDWORD)&pCallHook[dwCallIndex4+3]);
																						}
																						else
																						{
																							GetPPD(iItem, hwndDlg)->dwValue = (DWORD)pCallHook[dwCallIndex4+3];
																						}
...
VOID scanTrackingCharTypeOffset_Init(HWND hwndList)
{
	...
	ppd->dwValue = 0;
	ppd->pfnScanProc = scanTrackingCharTypeOffset_ScanProc;
