#include "scan_getguiddataconstant2.h"
#include "scan_common.h"
#include <commctrl.h>

#define SCAN_CATEGORY "BWH"
#define SCAN_NAME     "GetGuidDataConstant2"
#define SCAN_REQUIRED FALSE

VOID scanGetGuidDataConstant2_ScanProc(INT iItem, HWND hwndDlg, PBYTE pbFile)
{
								...
								for(DWORD dwCodeIndex2 = dwCodeIndex - 2; dwCodeIndex - dwCodeIndex2 < 20; dwCodeIndex2--)
								{
									if(*((PWORD)&pbCode[dwCodeIndex2]) == 0x5052 && pbCode[dwCodeIndex2 - 5] == 0xE8)
									{
										...
										for(DWORD dwCodeIndex3 = dwCodeIndex2 - 5; dwCodeIndex2 - dwCodeIndex3 < 20; dwCodeIndex3--)
										{
											...
											{
												// pull constant out of 'push constant' instruction
												GetPPD(iItem, hwndDlg)->dwValue = *((PDWORD)&pbCode[dwCodeIndex3 + 1]);
...
VOID scanGetGuidDataConstant2_Init(HWND hwndList)
{
	...
	ppd->dwValue = 0;
	ppd->pfnScanProc = scanGetGuidDataConstant2_ScanProc;
