#include "scan_trackingjumpsize.h"
#include "scan_common.h"
#include <commctrl.h>

#define SCAN_CATEGORY "BWH"
#define SCAN_NAME     "TrackingJumpSize"
#define SCAN_REQUIRED FALSE

VOID scanTrackingJumpSize_ScanProc(INT iItem, HWND hwndDlg, PBYTE pbFile)
{
																			...
																			if(pCallHook[dwCallIndex2] == 0x74 && pCallHook[ pCallHook[dwCallIndex2 + 1] + dwCallIndex2 + 1] == 0xC3)
																			{
																				...
																				if(dwCallIndex3 >= dwCallIndex + 6)
																				{
																					...
																					if(dwCallIndex4 < 100)
																					{
																						...
																						// scan for jump just above the hooked call
																						for(DWORD dwCodeIndex9 = dwCodeIndex6 - 5; dwCodeIndex9; dwCodeIndex9--)
																						{
																							if(pbCode[dwCodeIndex9] == 0x74 && pbCode[dwCodeIndex9+1] == dwCodeIndex6 - (dwCodeIndex9 + 2))
																							{
																								// found the jump, return number of bytes from instruction following jump to instruction following hooked call
																								GetPPD(iItem, hwndDlg)->dwValue = (dwCodeIndex6 + 5) - (dwCodeIndex9 + 2);
...
VOID scanTrackingJumpSize_Init(HWND hwndList)
{
	...
	ppd->dwValue = 0;
	ppd->pfnScanProc = scanTrackingJumpSize_ScanProc;
