#include "scan_trackinghook.h"
#include "scan_common.h"
#include <commctrl.h>

#define SCAN_CATEGORY "BWH"
#define SCAN_NAME     "TrackingHook"
#define SCAN_REQUIRED FALSE

VOID scanTrackingHook_ScanProc(INT iItem, HWND hwndDlg, PBYTE pbFile)
{
																			...
																			if(pCallHook[dwCallIndex2] == 0x74 && pCallHook[ pCallHook[dwCallIndex2 + 1] + dwCallIndex2 + 1] == 0xC3)
																			{
																				...
																				if(dwCallIndex3 >= dwCallIndex + 6)
																				{
																					...
																					if(dwCallIndex4 < 100)
																					{
																						...
																						// return va of relative offset part of hooked call
																						GetPPD(iItem, hwndDlg)->dwValue = OffsetToRva(pbFile, PtrToUlong(pbCode) + dwCodeIndex6 + 1 - PtrToUlong(pbFile)) + pNtHdr->OptionalHeader.ImageBase;
...
VOID scanTrackingHook_Init(HWND hwndList)
{
	...
	ppd->dwValue = 0;
	ppd->pfnScanProc = scanTrackingHook_ScanProc;
